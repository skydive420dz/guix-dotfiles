...
(ert-deftest sk/check-dialect-modules-participate-in-config-reload ()
  (let ((lisp-position (seq-position sk/reload-module-files "sk-lisp"))
        (studio-position
         (seq-position sk/reload-module-files "sk-scheme-studio"))
    ...
    (should studio-position)
    (should clojure-position)
    (should racket-position)
    (should fennel-position)
    (should format-position)
    (should (< lisp-position studio-position))
    (should (< studio-position clojure-position))
...
(ert-deftest sk/check-keyboard-quit-dwim-contexts ()
  (let (action)
    (cl-letf (((symbol-function 'minibufferp) (lambda () t))
              ((symbol-function 'minibuffer-keyboard-quit)
               (lambda () (setq action 'minibuffer))))
      (sk/keyboard-quit-dwim))
    (should (eq action 'minibuffer)))
  (dolist (state '(insert replace visual operator normal))
    (let ((evil-local-mode t)
      ...
      (cl-letf (((symbol-function 'minibufferp) (lambda () nil))
                ...
                ((symbol-function 'evil-force-normal-state)
                 (lambda () (setq action state))))
        (sk/keyboard-quit-dwim))
      (should (eq action state))))
  (let ((evil-local-mode nil)
        action)
    (cl-letf (((symbol-function 'minibufferp) (lambda () nil))
              ((symbol-function 'region-active-p) (lambda () t))
              ((symbol-function 'deactivate-mark)
               (lambda () (setq action 'region))))
      (sk/keyboard-quit-dwim))
    (should (eq action 'region)))
  (let ((evil-local-mode nil)
        action)
    (cl-letf (((symbol-function 'minibufferp) (lambda () nil))
              ((symbol-function 'region-active-p) (lambda () nil))
              ((symbol-function 'get-buffer-window)
               (lambda (&rest _) 'completion-window))
              ((symbol-function 'quit-window)
               (lambda (&optional _kill window) (setq action window))))
      (sk/keyboard-quit-dwim))
    (should (eq action 'completion-window)))
  (with-temp-buffer
    (help-mode)
    (let ((evil-local-mode nil)
          action)
      (cl-letf (((symbol-function 'minibufferp) (lambda () nil))
                ...
                ((symbol-function 'get-buffer-window) (lambda (&rest _) nil))
                ((symbol-function 'quit-window)
                 (lambda (&rest _) (setq action 'helper))))
        (sk/keyboard-quit-dwim))
      (should (eq action 'helper))))
  (with-temp-buffer
    (let ((evil-local-mode nil)
          action)
      (cl-letf (((symbol-function 'minibufferp) (lambda () nil))
                ...
                ((symbol-function 'keyboard-quit)
                 (lambda () (setq action 'fallback))))
        (sk/keyboard-quit-dwim))
      (should (eq action 'fallback)))))

(ert-deftest sk/check-keyboard-quit-bindings-cover-evil-and-global-maps ()
  (should (eq (lookup-key global-map (kbd "C-g"))
              #'sk/keyboard-quit-dwim))
  ...
  (dolist (map (list evil-normal-state-map
    ...
    (should (eq (lookup-key map (kbd "C-g"))
                #'sk/keyboard-quit-dwim))
    (should (eq (lookup-key map (kbd "<escape>"))
                #'sk/keyboard-quit-dwim))))

(ert-deftest sk/check-exwm-send-next-preserves-emacs-quoted-insert ()
  (require 'sk-exwm)
  (let (called)
    (cl-letf (((symbol-function 'derived-mode-p) (lambda (&rest _) t))
              ((symbol-function 'call-interactively)
               (lambda (command &rest _) (setq called command))))
      (sk/exwm-send-next-key))
    (should (eq called #'exwm-input-send-next-key)))
  (let (called)
    (cl-letf (((symbol-function 'derived-mode-p) (lambda (&rest _) nil))
              ((symbol-function 'call-interactively)
               (lambda (command &rest _) (setq called command))))
      (sk/exwm-send-next-key))
    (should (eq called #'quoted-insert))))

(ert-deftest sk/check-exwm-key-contract-is-unique-and-34-key-aligned ()
  (require 'sk-exwm)
  (let* ((descriptions (mapcar #'car sk/exwm-global-key-contract))
         (bindings (sk/exwm-key-bindings))
         (parsed-keys (mapcar #'car bindings)))
    (should (= (length descriptions)
               (length (delete-dups (copy-sequence descriptions)))))
    (should (= (length parsed-keys)
               (length (delete-dups (copy-sequence parsed-keys)))))
    (dolist (entry
  ...
  (let ((exwm--connection nil))
    (sk/exwm-bind-keys)
    (let ((first-value (copy-tree exwm-input-global-keys))
          (first-grabs (copy-tree exwm-input--global-keys)))
      (sk/exwm-bind-keys)
      (should (equal exwm-input-global-keys first-value))
      (should (equal exwm-input--global-keys first-grabs))
...
(ert-deftest sk/check-exwm-desktop-entry-is-read-once-per-spec ()
  (require 'sk-exwm)
  (let ((desktop-file (make-temp-file "sk-launcher-once." nil ".desktop"))
    ...
    (unwind-protect
        (progn
          ...
          (cl-letf (((symbol-function 'counsel-linux-apps-list-desktop-files)
                     (lambda () (list (cons "one.desktop" desktop-file))))
                    ((symbol-function 'executable-find)
                     (lambda (program)
                       (and (equal program "mock-app") "/mock/mock-app")))
                    ((symbol-function 'insert-file-contents)
                     (lambda (file &rest arguments)
                       (when (equal (file-truename file)
                                    (file-truename desktop-file))
                         (setq reads (1+ reads)))
                       (apply original-insert file arguments))))
            (should
             (equal (plist-get
                     (sk/exwm-desktop-launch-spec "one.desktop") :command)
                    '("/mock/mock-app" "--flag")))))
      (delete-file desktop-file))
    (should (= reads 1))))

(ert-deftest sk/check-exwm-launcher-reuses-validated-spec-cache ()
  (require 'sk-exwm)
  (let ((first (make-temp-file "sk-launcher-first." nil ".desktop"))
        (second (make-temp-file "sk-launcher-second." nil ".desktop"))
        (sk/exwm-launcher-cache nil)
        (parse-count 0))
    (unwind-protect
        (progn
          ...
          (with-temp-file second
            (insert "[Desktop Entry]\nName=Second\nExec=second-app\n"))
          (let* ((desktop-files (list (cons "first.desktop" first)
                                      (cons "second.desktop" second)))
                 (candidates
                  (list (cons (propertize "First" 'visible t) "first.desktop")
                        (cons (propertize "Second" 'visible t) "second.desktop")
                        (cons "Invisible duplicate" "second.desktop")))
                 (original-spec
                  (symbol-function 'sk/exwm-desktop-launch-spec)))
            (cl-letf
                (((symbol-function 'counsel-linux-apps-list-desktop-files)
                  (lambda () desktop-files))
                 ...
                 ((symbol-function 'sk/exwm-desktop-launch-spec)
                  (lambda (&rest arguments)
                    ...
                    (apply original-spec arguments))))
              (let ((cold (sk/exwm-launcher-state))
                    (warm (sk/exwm-launcher-state)))
                (should (eq cold warm))
                (should (= (length (plist-get warm :candidates)) 2))
                (should (= parse-count 2)))
              (set-file-times first (time-add (current-time) 2))
              (sk/exwm-launcher-state)
              (should (= parse-count 4)))))
...
(ert-deftest sk/check-startup-frame-geometry-detects-configure-events ()
  (let ((small-frame 'selected-frame)
        (active-frames '(selected-frame)))
    (cl-letf (((symbol-function 'frame-list)
               (lambda () '(selected-frame inactive-frame)))
              ...
              ((symbol-function 'frame-monitor-attribute)
               (lambda (attribute &optional _frame _x _y)
                 (should (eq attribute 'geometry))
                 '(0 0 3440 1440)))
              ((symbol-function 'frame-parameter)
               (lambda (frame parameter)
                 (pcase parameter
                   ('fullscreen 'fullboth)
                   ('exwm-active (memq frame active-frames))
                   (_ (error "unexpected frame parameter: %S" parameter)))))
              ((symbol-function 'frame-pixel-width)
               (lambda (frame)
                 (if (eq frame small-frame)
                     660
                   3440)))
              ((symbol-function 'frame-pixel-height)
               (lambda (frame)
                 (if (eq frame small-frame)
                     612
                   1440))))
      (should-not (sk/startup-frame--geometry-ready-p))
      ;; Once the selected frame settles, a cached inactive-workspace size does
      ;; not block the reveal; EXWM resizes it when that workspace activates.
      (setq small-frame 'inactive-frame)
      (should (sk/startup-frame--geometry-ready-p))
      ;; If EXWM marks more than one workspace visible, every visible frame
      ;; must settle before the opacity gate opens.
      (setq active-frames '(selected-frame inactive-frame))
      (should-not (sk/startup-frame--geometry-ready-p))
      (setq small-frame nil)
      (should (sk/startup-frame--geometry-ready-p)))))

(ert-deftest sk/check-startup-frame-scheduler-yields-and-polls ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-release-timer nil)
    ...
    (should (= sk/startup-frame-geometry-deadline 102.0))
    (should (eq sk/startup-frame-gate-release-timer
                'idle-release-timer))
    (should
     (equal idle-call
            (list 0 nil #'sk/startup-frame-release nil)))
    (should-not
     (memq #'sk/startup-frame-schedule-release window-setup-hook)))
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-release-timer 'idle-release-timer)
        (sk/startup-frame-geometry-deadline 102.0)
        retry-call
        canceled)
    (cl-letf (((symbol-function 'timerp)
               (lambda (object)
                 (memq object '(idle-release-timer retry-timer))))
              ((symbol-function 'cancel-timer)
               (lambda (timer)
                 (setq canceled timer)))
              ((symbol-function 'float-time)
               (lambda (&optional _time) 100.0))
              ((symbol-function 'sk/startup-frame--geometry-ready-p)
               (lambda () nil))
              ...
              ((symbol-function 'redraw-display)
               (lambda ()
                 (error "polling must not reveal"))))
      (sk/startup-frame-release))
    (should (eq canceled 'idle-release-timer))
    (should (eq sk/startup-frame-gate-release-timer 'retry-timer))
    (should
     (equal retry-call
            (list 0.05 nil #'sk/startup-frame-release nil)))
    (should sk/startup-frame-gate-active-p)
    (should-not sk/startup-frame-gate-release-complete-p)))

(ert-deftest sk/check-startup-frame-retry-completes-after-configure-event ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-release-timer 'idle-release-timer)
        (sk/startup-frame-geometry-deadline 102.0)
        (geometry-ready-p nil)
        (finish-count 0)
        canceled-timers)
    (cl-letf (((symbol-function 'timerp)
               (lambda (object)
              ...
              ((symbol-function 'cancel-timer)
               (lambda (timer)
                 (setq canceled-timers
                       (append canceled-timers (list timer)))))
              ((symbol-function 'float-time)
               (lambda (&optional _time) 100.0))
              ((symbol-function 'sk/startup-frame--geometry-ready-p)
               (lambda () geometry-ready-p))
              ((symbol-function 'run-at-time)
               (lambda (_seconds _repeat _function &rest _arguments)
                 'retry-timer))
              ((symbol-function 'sk/startup-frame--finish-release)
               (lambda ()
                 (setq finish-count (1+ finish-count)
                       sk/startup-frame-gate-active-p nil
                       sk/startup-frame-gate-release-complete-p t
                       sk/startup-frame-geometry-deadline nil))))
      (sk/startup-frame-release)
      (should (eq sk/startup-frame-gate-release-timer 'retry-timer))
      (should-not sk/startup-frame-gate-release-complete-p)
      (setq geometry-ready-p t)
      (sk/startup-frame-release))
    (should (= finish-count 1))
    (should (equal canceled-timers
                   '(idle-release-timer retry-timer)))
    (should-not sk/startup-frame-gate-active-p)
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-release-timer)
    (should-not sk/startup-frame-geometry-deadline)))

(ert-deftest sk/check-startup-frame-idle-registration-fails-open ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-release-timer nil)
        (sk/startup-frame-geometry-deadline nil)
    ...
    (cl-letf (((symbol-function 'float-time)
              ...
              ((symbol-function 'run-with-idle-timer)
               (lambda (&rest _arguments)
                 (error "deliberate idle registration failure")))
              ((symbol-function 'sk/startup-frame--warn)
               (lambda (format-string error-data)
                 (setq warning
      ...
      (sk/startup-frame-schedule-release))
    (should (= finish-count 1))
    (should (string-match-p "idle release" warning))
    (should-not sk/startup-frame-gate-active-p)
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-release-timer)
    (should-not sk/startup-frame-geometry-deadline)
...
(ert-deftest sk/check-startup-frame-retry-registration-fails-open ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-release-timer 'idle-release-timer)
        (sk/startup-frame-geometry-deadline 102.0)
    ...
    (cl-letf (((symbol-function 'timerp)
              ...
              ((symbol-function 'run-at-time)
               (lambda (&rest _arguments)
                 (error "deliberate retry registration failure")))
              ((symbol-function 'sk/startup-frame--warn)
               (lambda (format-string error-data)
                 (setq warning
                       (format format-string
    ...
    (should (= finish-count 1))
    (should (eq canceled 'idle-release-timer))
    (should (string-match-p "geometry retry" warning))
    (should-not sk/startup-frame-gate-active-p)
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-release-timer)
    (should-not sk/startup-frame-geometry-deadline)))

(ert-deftest sk/check-startup-frame-watchdog-follows-x-startup-intent ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-watchdog-timer nil)
        (initial-window-system 'x)
        scheduled)
    (cl-letf (((symbol-function 'timerp)
               (lambda (object)
                 (eq object 'watchdog-timer)))
              ((symbol-function 'run-at-time)
               (lambda (seconds repeat function &rest arguments)
                 (setq scheduled
                       (append scheduled
      ...
      (sk/startup-frame--arm-watchdog))
    (should (eq sk/startup-frame-gate-watchdog-timer 'watchdog-timer))
    (should
     (equal scheduled
            (list
             (list sk/startup-frame-gate-watchdog-seconds nil
                   #'sk/startup-frame--watchdog-release nil)))))
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-gate-watchdog-timer nil)
        (sk/startup-frame-geometry-deadline 999.0)
        (initial-window-system 'x)
        (finish-count 0)
        warning)
    (cl-letf (((symbol-function 'run-at-time)
      ...
      (sk/startup-frame--arm-watchdog))
    (should (= finish-count 1))
    (should (string-match-p "watchdog" warning))
    (should-not sk/startup-frame-gate-active-p)
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-watchdog-timer)
    (should-not sk/startup-frame-geometry-deadline))
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-watchdog-timer nil)
    ...
    (cl-letf (((symbol-function 'run-at-time)
      ...
      (sk/startup-frame--arm-watchdog))
    (should (= schedule-count 0))
    (should-not sk/startup-frame-gate-watchdog-timer)))

(ert-deftest sk/check-startup-frame-watchdog-bypasses-geometry-polling ()
  (let ((sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-geometry-deadline 999.0)
        (finish-count 0)
        warning)
    (cl-letf (((symbol-function 'sk/startup-frame--geometry-ready-p)
               (lambda ()
                 (error "watchdog must not inspect geometry")))
              ((symbol-function 'sk/startup-frame--warn-text)
               (lambda (message)
...
(ert-deftest sk/check-startup-frame-expired-deadline-reveals-and-cleans ()
  (let ((process-environment (copy-sequence process-environment))
        (sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-final-opacity-percent 85)
        (sk/startup-frame-saved-alpha-lower-limit 20)
        (sk/startup-frame-gate-watchdog-timer 'watchdog-timer)
        (sk/startup-frame-gate-release-timer 'release-timer)
        (sk/startup-frame-geometry-deadline 99.0)
        (frame-alpha-lower-limit 0)
        (initial-frame-alist '((alpha . 0) (fullscreen . fullboth)))
        (default-frame-alist '((alpha . 0) (undecorated . t)))
        (window-setup-hook
         (list #'sk/startup-frame-schedule-release))
        canceled-timers
        opacity
        warning)
    (setenv sk/startup-frame-opacity-environment-variable "85")
    (cl-letf (((symbol-function 'selected-frame)
              ...
              ((symbol-function 'set-frame-parameter)
               (lambda (_frame parameter value)
                 (should (eq parameter 'alpha))
                 (setq opacity value)))
              ((symbol-function 'timerp)
               (lambda (object)
                 (memq object '(release-timer watchdog-timer))))
              ((symbol-function 'cancel-timer)
               (lambda (timer)
                 (setq canceled-timers
                       (append canceled-timers (list timer)))))
      ...
      (sk/startup-frame-release))
    (should (= opacity 85))
    (should (string-match-p "geometry did not settle" warning))
    (should (equal canceled-timers
                   '(release-timer watchdog-timer)))
    (should-not sk/startup-frame-gate-active-p)
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-release-timer)
    (should-not sk/startup-frame-gate-watchdog-timer)
    ...
    (should-not (assq 'alpha initial-frame-alist))
    (should-not (assq 'alpha default-frame-alist))
    (should-not
     (getenv sk/startup-frame-opacity-environment-variable))
    (should-not
     (memq #'sk/startup-frame-schedule-release window-setup-hook))))

(ert-deftest sk/check-startup-frame-opacity-fallback-is-opaque-and-warning-safe ()
  (let (opacity-calls)
    (cl-letf (((symbol-function 'frame-live-p)
               (lambda (_frame) t))
              ((symbol-function 'display-graphic-p)
              ...
              ((symbol-function 'set-frame-parameter)
               (lambda (_frame parameter value)
                 (should (eq parameter 'alpha))
                 (setq opacity-calls
                       (append opacity-calls (list value)))
                 (when (= value 85)
                   (error "deliberate target-opacity failure"))))
              ((symbol-function 'display-warning)
               (lambda (&rest _arguments)
                 (error "deliberate warning failure"))))
      (should (sk/startup-frame--set-opacity 'fixture-frame 85)))
    (should (equal opacity-calls '(85 100)))))

(ert-deftest sk/check-startup-frame-release-is-ordered-and-one-shot ()
  (let ((process-environment (copy-sequence process-environment))
        (sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-final-opacity-percent 85)
        ...
        (frames '(selected-frame inactive-frame-1 inactive-frame-2))
        opacity-calls
        canceled-timers
        events)
    (setenv sk/startup-frame-opacity-environment-variable "85")
    (cl-letf (((symbol-function 'selected-frame)
              ...
              ((symbol-function 'set-frame-parameter)
               (lambda (frame parameter value)
                 (setq events (append events (list 'opacity)))
                 (setq opacity-calls
                       (append opacity-calls
                               (list (list frame parameter value))))))
              ((symbol-function 'sk/startup-frame--geometry-ready-p)
               (lambda ()
                 (setq events (append events (list 'geometry-ready)))
                 t))
              ((symbol-function 'redraw-display)
               (lambda ()
                 (setq events (append events (list 'redraw)))))
              ((symbol-function 'timerp)
               (lambda (object)
                 (memq object '(release-timer watchdog-timer))))
              ((symbol-function 'cancel-timer)
      ...
      (should
       (equal events
              '(geometry-ready redraw opacity opacity opacity)))
      (should
       (equal opacity-calls
              '((inactive-frame-1 alpha 85)
                (inactive-frame-2 alpha 85)
                (selected-frame alpha 85))))
      (should-not (assq 'alpha initial-frame-alist))
      (should-not (assq 'alpha default-frame-alist))
      ...
      (should-not sk/startup-frame-gate-release-timer)
      (should-not sk/startup-frame-geometry-deadline)
      ;; A stale caller cannot change opacity or redraw twice.
      (sk/startup-frame-release)
      (should (= (length opacity-calls) 3))
      (should (= (length events) 5)))))

(ert-deftest sk/check-startup-frame-release-fails-open-after-redraw-error ()
  (let ((process-environment (copy-sequence process-environment))
        (sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-final-opacity-percent 85)
        ...
        (sk/startup-frame-geometry-deadline nil)
        (frame-alpha-lower-limit 0)
        (initial-frame-alist '((alpha . 0)))
        (default-frame-alist '((alpha . 0)))
        (window-setup-hook (list #'sk/startup-frame-schedule-release))
        opacity
        warning)
    (setenv sk/startup-frame-opacity-environment-variable "85")
    (cl-letf (((symbol-function 'selected-frame)
              ...
              ((symbol-function 'redraw-display)
               (lambda ()
                 (error "deliberate pre-render failure")))
              ((symbol-function 'set-frame-parameter)
               (lambda (_frame parameter value)
                 (should (eq parameter 'alpha))
                 (setq opacity value)))
      ...
      (sk/startup-frame-release))
    (should (= opacity 85))
    (should (string-match-p "Could not pre-render" warning))
    (should sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-active-p)
    (should (= frame-alpha-lower-limit 20))
    (should-not (assq 'alpha initial-frame-alist))
...
(ert-deftest sk/check-startup-frame-release-forces-all-frames-open ()
  (let ((process-environment (copy-sequence process-environment))
        (sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-final-opacity-percent 85)
    ...
    (cl-letf (((symbol-function 'selected-frame)
              ...
              ((symbol-function 'set-frame-parameter)
               (lambda (frame parameter value)
                 (should (eq parameter 'alpha))
                 (setq opacity-calls
                       (append opacity-calls (list (list frame value))))
                 (when (and (eq frame 'inactive-frame)
                            (= value 100)
                            (= (cl-incf inactive-fallback-count) 1))
                   (error "deliberate first fallback failure"))))
              ((symbol-function 'display-warning) #'ignore))
      ;; The target write itself must fail before its first opaque fallback.
      (cl-letf (((symbol-function 'set-frame-parameter)
                 (let ((underlying (symbol-function 'set-frame-parameter)))
                   (lambda (frame parameter value)
                     (if (and (eq frame 'inactive-frame)
                              (= value 85))
                         (progn
                           (setq opacity-calls
                                 (append opacity-calls
                                         (list (list frame value))))
                           (error "deliberate target failure"))
                       (funcall underlying frame parameter value))))))
        (sk/startup-frame-release)))
    (should
     (equal opacity-calls
            '((inactive-frame 85)
...
(ert-deftest sk/check-startup-frame-release-stops-picom-as-last-resort ()
  (let ((process-environment (copy-sequence process-environment))
        (sk/startup-frame-gate-active-p t)
        (sk/startup-frame-gate-release-complete-p nil)
        (sk/startup-frame-final-opacity-percent 85)
    ...
    (cl-letf (((symbol-function 'selected-frame)
              ...
              ((symbol-function 'set-frame-parameter)
               (lambda (_frame parameter value)
                 ...
                 (setq opacity-calls
                       (append opacity-calls (list value)))
                 (error "deliberate persistent opacity failure")))
              ((symbol-function 'executable-find)
               (lambda (program)
                 (and (equal program "pkill") "/mock/pkill")))
              ...
              ((symbol-function 'display-warning)
               (lambda (&rest _arguments)
                 (error "deliberate warning failure"))))
      (sk/startup-frame-release))
    (should (equal opacity-calls '(85 100 100)))
    (should
     (equal pkill-call
            (list
             "/mock/pkill" nil nil nil
             (list "-u" (number-to-string (user-uid)) "-x" "picom"))))
    (should-not sk/startup-frame-gate-release-complete-p)
    (should-not sk/startup-frame-gate-active-p)
    (should (= frame-alpha-lower-limit 20))
    (should-not (assq 'alpha initial-frame-alist))
    ...
    (cl-letf (((symbol-function 'sk/exwm-assert-compatible) #'ignore)
              ...
              ((symbol-function 'exwm-wm-mode)
               (lambda (&optional _argument)
                 (error "deliberate EXWM registration failure"))))
      (should-error (sk/exwm-start) :type 'error))
    (should
...
(ert-deftest sk/check-start-picom-isolates-ambient-config ()
  ...
  (let ((pkill-count 0)
               ...
               (lambda (program &optional infile destination display &rest args)
                 (pcase program
                   ...
                   ("picom"
                    ...
                    (setq events (append events '(picom-daemon)))
                    (setq start-call
                          (list program infile destination display args))
                    ;; `--daemon' returns only after Picom initializes; expose
                    ;; the replacement process and selection to the readiness
                    ;; check that follows the synchronous call.
                    (setq picom-active t
                          selection-owned t)
                    0)
                   (_
                    (error "unexpected call-process program: %s" program)))))
    ...
    (should
     (equal events '(pkill wait-process wait-selection picom-daemon)))
...
(ert-deftest sk/check-start-picom-reports-daemon-initialization-failure ()
  (require 'sk-exwm)
  (let ((window-system 'x)
        picom-call)
    (cl-letf (((symbol-function 'executable-find)
              ...
              ((symbol-function 'call-process)
               (lambda (program &optional infile destination display
                                &rest arguments)
                 (if (equal program "pkill")
                     1
                   (setq picom-call
                         (list program infile destination display arguments))
                   42)))
              ((symbol-function 'sk/picom--wait-for-stop)
               (lambda () t)))
      (let ((failure (should-error (sk/start-picom) :type 'user-error)))
        (should
         (string-match-p "failed to initialize (status 42)"
                         (error-message-string failure)))))
    (should
     (equal
      picom-call
...
(ert-deftest sk/check-start-picom-cleans-up-after-readiness-failure ()
  (require 'sk-exwm)
  (let ((window-system 'x)
        calls)
    (cl-letf (((symbol-function 'executable-find)
              ...
              ((symbol-function 'call-process)
               (lambda (program &rest _arguments)
                 (setq calls (append calls (list program)))
                 (if (equal program "picom") 0 1)))
              ((symbol-function 'sk/picom--wait-for-stop)
               (lambda () t))
              ((symbol-function 'sk/picom--wait-for-ready)
               (lambda () nil)))
      (let ((failure (should-error (sk/start-picom) :type 'user-error)))
        (should
         (string-match-p "did not own the compositor selection"
                         (error-message-string failure)))))
    (should (equal calls '("pkill" "picom" "pkill")))))
